...
internal class CoursesDataHelper
{
    private static readonly List<Courses> AllCourses = new()
    {
        // Standard Courses
    ...
    internal async Task<Courses> GetCourse(int? standardCode = null)
    {
        await Task.Delay(50);
        return standardCode.HasValue
            ? AllCourses.FirstOrDefault(c => c.StandardCode == standardCode.Value)
              ?? await GetRandomTestCourse()
            : await GetRandomTestCourse();
    }

    internal async Task<Courses> GetRandomTestCourse()
    {
        await Task.Delay(50);
        return AllCourses
    ...
    internal async Task<Courses> GetRandomFoundationCourse()
    {
        await Task.Delay(50);
        return AllCourses
